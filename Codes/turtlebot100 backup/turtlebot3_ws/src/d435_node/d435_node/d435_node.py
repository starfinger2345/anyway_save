...
#        webcam_path = "/dev/v4l/by-id/usb-Innomaker_Innomaker-U20CAM-1080p-S1_SN0001-video-index0"
        ...
        if not self.cap.isOpened():
            self.get_logger().error(f"Can't open webcam: /dev/video6")
            self.cap = None
        ...
        try:
            ...
            if self.cds < 30:
                ...
                if not ir_frame:
                    ...
                    M = cv2.getPerspectiveTransform(ir_mapped_pts, self.rgb_corners)
            ...
            if self.cap is not None:
                ...
                if ret:
                    # 기존 (self.width, self.height) 대신 절반 크기로 리사이즈
                    webcam_resized = cv2.resize(webcam_frame, (resized_width, resized_height))
                else:
                    webcam_resized = np.zeros((resized_height, resized_width, 3), dtype=np.uint8)
            else:
                webcam_resized = np.zeros((resized_height, resized_width, 3), dtype=np.uint8)
            
            # --- 리얼센스 처리 부분 (이 부분도 같이 줄여야 합니다) ---
            # 위쪽 if/else 문에서 생성된 depth_color_frame을 320x240으로 리사이즈
            depth_color_resized = cv2.resize(depth_color_frame, (resized_width, resized_height))
                
            # --- 두 영상 합치기 ---
            # 이제 둘 다 320x240이므로 가로로 붙이면 640x240의 이미지가 됨.
            combined_image = np.hstack((webcam_resized, depth_color_resized))
