...
// [추가] 리프트용 다이나믹셀 ID 정의 (하드코딩 방지)
const uint8_t DXL_MOTOR_ID_LIFT_LEFT = 5;
const uint8_t DXL_MOTOR_ID_LIFT_RIGHT = 6;
...
bool Turtlebot3MotorDriver::init(void)
{
  ...
  // [여기에 추가!] 리프트 모터(5번, 6번) 토크 켜기
  // 다이나믹셀 프로토콜 2.0 명령으로 직접 토크(Torque Enable)를 켭니다.
  dxl.writeControlTableItem(TORQUE_ENABLE, DXL_MOTOR_ID_LIFT_LEFT, 1);
  dxl.writeControlTableItem(TORQUE_ENABLE, DXL_MOTOR_ID_LIFT_RIGHT, 1);
...
// [추가] 리프트 모터 구동 함수 구현
bool Turtlebot3MotorDriver::write_lift_angle(int32_t goal_angle)
{
    bool ret = false;

    // 위치 제어를 위한 SyncWrite 파라미터 임시 생성 (116 = Goal Position 주소, 4바이트 길이)
    ParamForSyncWriteInst_t sync_write_pos_param;
    sync_write_pos_param.addr = 116;
    sync_write_pos_param.length = 4;
    sync_write_pos_param.id_count = 2; // 리프트 모터 2개 사용

    // 리프트 모터 ID (기구 설계에 맞춰 5번, 6번으로 지정)
    sync_write_pos_param.xel[0].id = DXL_MOTOR_ID_LIFT_LEFT;
    sync_write_pos_param.xel[1].id = DXL_MOTOR_ID_LIFT_RIGHT;

    // 안전 범위 제한 (0 ~ 4095)
    int32_t safe_angle = goal_angle;
    if (safe_angle < 0) safe_angle = 0;
    if (safe_angle > 4095) safe_angle = 4095;

    // 모터가 서로 마주보고 설치되었다면, 한 쪽은 역방향(4095 - 각도)으로 움직여야 할 수 있습니다.
    int32_t left_lift_angle = safe_angle;
    int32_t right_lift_angle = safe_angle; // 반전이 필요하면 (4095 - safe_angle) 로 수정하세요.

    // 데이터 복사
    memcpy(sync_write_pos_param.xel[0].data, &left_lift_angle, sync_write_pos_param.length);
    memcpy(sync_write_pos_param.xel[1].data, &right_lift_angle, sync_write_pos_param.length);

    // SyncWrite로 동시에 전송!
    if (dxl.syncWrite(sync_write_pos_param)) {
        ret = true;
    }
