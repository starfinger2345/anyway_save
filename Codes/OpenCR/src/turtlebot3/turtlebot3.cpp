...
enum ControlTableItemAddr{
  ...
  ADDR_CUSTOM_LED_SERVICE = 182,
  ADDR_CMD_LIFT_ANGLE     = 186, // [추가] 리프트 제어용 186번 주소
...
typedef struct ControlItemVariables{
  ...
  int32_t cmd_lift_angle;  // [추가] 라즈베리파이가 보낸 각도를 저장할 변수
...
void TurtleBot3Core::begin(const char* model_name)
{
  ...
  dxl_slave.addControlItem(ADDR_CMD_LIFT_ANGLE, control_items.cmd_lift_angle); // [추가] 186번 주소 연결
...
static void dxl_slave_write_callback_func(uint16_t item_addr, uint8_t &dxl_err_code, void* arg)
{
  ...
  switch(item_addr)
  {
    ...
    case ADDR_CMD_VEL_ANGULAR_Z:
      ...
      // [추가] 라즈베리파이가 186번에 값을 썼을 때 모터 드라이버로 각도 전송!
    case ADDR_CMD_LIFT_ANGLE:
      motor_driver.write_lift_angle(control_items.cmd_lift_angle);
      break;
